...
// a couple of SDK revisions ship the header without the enum; SHCORE.dll is
// loaded at runtime (see app.cpp), so we only need the constant
...
#ifdef _MSC_VER        // MinGW/gcc gets these from build.ps1 (the -l flags)
...
// ---------- tunables ----------
// DSKV_VERSION_STR comes from include/version.h, which build.ps1 rewrites on every
// build (no -D on the command line: shells re-quote it, see that file's comment).
